  ...
  func irc_defaultMsgSend(_ message: IRCMessage) throws {
    do {
      switch message.command {
        ...
        case .NOTICE(let recipients, let noticeMessage):
          try doNotice(recipients: recipients, message: noticeMessage,
                       serverTime: message.serverTime)
...
public extension IRCDispatcher {
  ...
  func doNotice(recipients: [ IRCMessageRecipient ], message: String,
                serverTime: Date?) throws {
    throw InternalDispatchError.notImplemented(function: #function)
  }
  func doMessage(sender: IRCUserID?, recipients: [ IRCMessageRecipient ],
                 message: String, serverTime: Date?) throws
  {
    throw InternalDispatchError.notImplemented(function: #function)
  }
