...
public struct IRCMessageParser {
       ...
       -> IRCMessage
  {
    ...
    //   ['@' TAGS ' ']? [':' SOURCE]? ' ' COMMAND [' ' ARGS]? [' :' LAST-ARG]?
    ...
    if cursor[cursor.startIndex] == cAt {
      ...
      // Find the space that ends the tags section
      guard let spaceIdx = cursor.firstIndex(of: cSpace) else {
        throw Error.syntaxError
      }

      let tagsSlice = cursor[cursor.startIndex..<spaceIdx]
      if let tagsString = makeString(from: tagsSlice) {
        tags = [:]
        // Parse tag1=value1;tag2=value2
        for tagPair in tagsString.split(separator: ";") {
      ...
      cursor = cursor[spaceIdx..<cursor.endIndex]
      skipSpaces()
    }
    ...
    if !cursor.isEmpty && cursor[cursor.startIndex] == cColon {
