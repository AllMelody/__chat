...
public extension IRCCommand {
  ...
  init(_ command: String, arguments: [ String ]) throws {
    ...
    switch command.uppercased() {
      ...
      case "CAP":
        // CAP has two formats:
        // Client: CAP <subcommand> [:<capabilities>]
        // Server: CAP <nick> <subcommand> [*] [:<capabilities>]
        // We need to find the subcommand which could be at index 0 or 1
        try expect(min: 1)

        var subcmdIndex = 0
        var capsIndex = 1

        // Check if arguments[0] is a valid subcommand
        if CAPSubCommand(rawValue: arguments[0]) == nil && arguments.count >= 2 {
          // arguments[0] is likely the target nick (e.g., "*"), subcommand is at index 1
          subcmdIndex = 1
          capsIndex = 2
          // Check for multi-line indicator "*" at index 2
          if arguments.count > 2 && arguments[2] == "*" {
            capsIndex = 3
          }
        }

        guard arguments.count > subcmdIndex,
              let subcmd = CAPSubCommand(rawValue: arguments[subcmdIndex]) else {
          throw IRCParserError.invalidCAPCommand(arguments.count > subcmdIndex ? arguments[subcmdIndex] : "")
        }

        let capIDs = arguments.count > capsIndex
                   ? arguments[capsIndex].components(separatedBy: " ")
                   : []
